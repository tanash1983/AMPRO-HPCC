...
class BuildMixedAccountModels:
    ...
    def __init__(self,
        ...
        self.path_to_accinfo = path_to_accinfo
...
def BuildMARM():
    ...
    # check if path_to_accinfo is valid
    if not os.path.isfile(args.path_to_accinfo):
        print("Invalid path to account information! Exiting..")
        raise SystemExit
    ...
    bmarm = BuildMixedAccountModels(path_to_data=args.path_to_data, methodnames=methodnames,
                                    min_num_jobs=args.min_num_jobs, path_to_accinfo=args.path_to_accinfo,
                                    indep_var=indep_vars, dep_var=args.dep_vars)
    scores = bmarm.MARM()

    end_time = time.time()
    print("MARMs built and evaluated in " + str(end_time - start_time) + " seconds.")
