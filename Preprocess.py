...
class Preprocess:
    ...
    def fix_data(self):
        ...
        acc_factors, unique_acc = pd.factorize(self.data.Account)
        pd.DataFrame({'AccountHash': unique_acc,
                      'AccountFactor': range(len(unique_acc))}). \
            to_csv(self.output_dir + "/" + self.system_name + "AccountHash_Factors.csv")
        self.data['Account'] = acc_factors

        # Prepare 'QOS' factors
        qos_factors, unique_qos = pd.factorize(self.data['QOS'])
        pd.DataFrame({'QOS': unique_qos,
                      'QOSFactor': range(len(unique_qos))}). \
...
def PreprocessData():
    ...
    sys_def = pd.DataFrame({'SysName': args.sys_name,
                            'Time': args.def_time,
                            'QOS': args.def_qos,
                            'Partition': args.def_partition}, index=[0])
    sys_def.to_csv(args.output_dir + "/" + args.sys_name + "_" + "SysDefault.csv")
    ...
    end_time = time.time()
    print("Pre-processing completed in " + str(round(end_time - start_time, 2)) + " seconds.")
